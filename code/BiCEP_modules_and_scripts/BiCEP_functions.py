...
class SpecimenCollection():
    ...
    def histplot(self,ax,**kwargs):
        ...
        **kwargs:
        ...
        None
        """
        this_hist, _, _ = ax.hist(self.fit['int_site'],bins=100,color='skyblue',density=True)
        minB,maxB=np.percentile(self.fit['int_site'],(2.5,97.5))
        ax.plot([minB, minB], [0, this_hist.max()], 'grey', linestyle='dashed', linewidth=1)
        ax.plot([maxB, maxB], [0, this_hist.max()], 'grey', linestyle='dashed', linewidth=1)
        # ax.plot([minB,maxB],[0,0],'k',linewidth=4)
        ax.set_xlim(np.percentile(self.fit['int_site'],(1.25,98.75)))
